...
app.get('/health', async (req, res) => {
  ...
  try {
    ...
    health.services.socketio = {
      status: 'ok',
      message: `Active connections: ${connectedSockets}`
    };
  } catch (error) {
  ...
  health.services.memory = {
    ...
    message: `Heap: ${(memUsage.heapUsed / 1024 / 1024).toFixed(1)}MB / ${(memUsage.heapTotal / 1024 / 1024).toFixed(1)}MB`
  };

  if (health.services.memory.status === 'warning') {
    health.status = 'warning';
  }

  res.status(health.status === 'ok' ? 200 : health.status === 'warning' ? 200 : 503).json(health);
});
