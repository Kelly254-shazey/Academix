...
export default function LecturerControls({ onStart, onDelay, onCancel, onChangeRoom }){
        ...
        <div className="grid grid-cols-2 gap-2 sm:gap-3">
          {actionButtons.map((btn) => (
            <button
              key={btn.action}
              onClick={btn.handler}
              className={`px-3 sm:px-4 py-3 sm:py-3 bg-gradient-to-r ${btn.color} text-white text-xs sm:text-sm font-semibold rounded-lg hover:shadow-lg hover:scale-105 transition-all transform active:scale-95 flex items-center justify-center gap-2`}
            >
        ...
        {confirmOpen && (
          <div className="mt-4 p-4 sm:p-5 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-lg border-2 border-yellow-200 shadow-sm">
            ...
            <div className="mt-4 flex gap-2 sm:gap-3">
              <button 
                onClick={confirm} 
                className="flex-1 px-3 py-2 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white text-xs sm:text-sm font-semibold rounded-lg hover:shadow-lg transition-all"
              >
                ✓ Confirm
              </button>
              <button 
                onClick={()=>setConfirmOpen(false)} 
                className="flex-1 px-3 py-2 bg-gray-200 text-gray-700 text-xs sm:text-sm font-semibold rounded-lg hover:bg-gray-300 transition-all"
              >
                ✕ Cancel
              </button>
            </div>
